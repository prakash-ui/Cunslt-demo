...
async function getNotifications(limit: number) {
  ...
  return {
    notifications: [
      { id: 1, title: "New Booking", body: "You have a new booking.", type: "booking_created", is_read: false, timeAgo: "2h ago", link: "/bookings/1" },
      { id: 2, title: "Payment Received", body: "Your payment was successful.", type: "payment_received", is_read: true, timeAgo: "1d ago", link: "/payments/1" },
    ],
    count: 2,
  }
}
...
async function markAllNotificationsAsRead() {
  // Simulate an API call to mark all notifications as read
  return new Promise((resolve) => {
    setTimeout(() => {
...
async function archiveNotification(id: number) {
  // Simulate an API call to archive the notification
  return new Promise((resolve) => {
    setTimeout(() => {
      console.log(`Notification with ID ${id} archived.`);
      resolve(true);
    }, 500); // Simulate network delay
  });
}
